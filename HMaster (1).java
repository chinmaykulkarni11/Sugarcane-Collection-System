...
public class HMaster  extends JFrame implements ActionListener,ItemListener
{
	JFrame F;
	JLabel Season,Id,Name,VId,Addr,Contact,Shares,Rate,Joindate,MDate,S;		
	...
	public HMaster()
	{
		...
		F.setTitle("Harvester Operator");
		//F.setDefaultCloseOperation(F.EXIT_ON_CLOSE);
		F.setLayout(null);
		
		Season=new JLabel("Season");
		Id= new JLabel("Harvester id");
		Name= new JLabel("Harvester Name");								T4=new JTextField();			
		VId= new JLabel("Harvester Village Id");						T3=new JTextField();		
		Contact= new JLabel("Harvester Contact info");			    	T1=new JTextField();
		Shares= new JLabel("Harvester Number");							T2=new JTextField();
		Rate= new JLabel("Share rate");									T5=new JTextField();
      	Joindate= new JLabel("Harvester Joindate");						
		Addr=new JLabel("Harvester Address");
...
// combobox setup
		...
		Joindate.setBounds(90,450,300,80);	MDate.setBounds( 120,470,300,80);	T5.setBounds(310,470,150,50);
		...
		F.add(Shares);		F.add(T1);		F.add(MDate);	F.add(S);				
		...
		catch(Exception e)
		{
			...
			try
				{Combo2.removeAllItems();Combo1.removeAllItems();Combo3.removeAllItems();
					rs=stmt.executeQuery("select max(Hid) from HMaster"); 
					while(rs.next())
					{	flag=0;
					...
					T3.setText("");T2.setText("");T1.setText("");T4.setText("");
				}
				catch(Exception e)
				{
			...
			try
			{
				...
				if(evt.getSource()==Combo1 && evt.getStateChange()==ItemEvent.SELECTED  && flag==1)
					{	
						...
						stmt=con.createStatement();
						rs=stmt.executeQuery("select * from HMaster where Hid="+temp);
						if(rs.next())
						{flag=0;
					...
					{	
						...
						if(i.length()>0)
						{stmt=con.createStatement();
						rs=stmt.executeQuery("select * from HMaster where Hname='"+i+"'");
						if(rs.next())
						{flag=0;
							T1.setText(rs.getString(4)+"");
							T2.setText(rs.getString(5)+"");
					...
					{	
						...
						if(i.length()>0)
						{stmt=con.createStatement();
						rs=stmt.executeQuery("select * from HMaster where HVid='"+i+"'");
						if(rs.next())
						{flag=0;
							T1.setText(rs.getString(4)+"");
							T2.setText(rs.getString(5)+"");
		...
		public void actionPerformed(ActionEvent evt)
{
		...
		if (evt.getSource()==New)
		{
			try
				{Combo2.removeAllItems();Combo1.removeAllItems();Combo3.removeAllItems();
					...
					T3.setText("");T2.setText("");T1.setText("");T4.setText("");T5.setText("");
					SimpleDateFormat sdf=new SimpleDateFormat("yyyy/MM/dd");
			Date date=new Date();
			T5.setText(sdf.format(date));
				}
		...
		{	 int r=0; String z=Search.getText();
			  if( z.equals("Search"))	
			{
				try
				{
					rs=stmt.executeQuery("select Hid,Hname,HVid from HMaster"); 
					...
					String TX1=T1.getText();String TX2=T2.getText();String TX3=T3.getText();String TX4=T4.getText(); String TX5=T5.getText(); 
						...
						if(TX2.length()==10)
									...
									if(U1.length()>0 && U2.length()>0 && U3.length()>0)
										{
	int r1=stmt.executeUpdate("insert into HMaster values("+temp1+",'"+U2+"',"+temp2+",'"+TX1+"',"+temp3+","+temp4+","+temp5+",'"+TX5+"')");
	...
	if(evt.getSource()==Delete)
			...
			if( z.equals("Delete"))	
			{
				try{
						...
						{
							stmt=con.createStatement();
							int r1=stmt.executeUpdate("Delete from HMaster where Hid="+temp1);
	...
	//int r1=stmt.executeUpdate("Update MMaster Set Mname='"+U2+"',MVid="+temp2+"  where Mid="+temp1);
		int r1=stmt.executeUpdate("Update HMaster Set Hname='"+U2+"',HVid="+temp2+" ,HAddr='"+TX1+"',HContact='"+temp3+"',HNShares='"+temp4+"',HSRate="+temp5+", HRMDate='"+TX5+"' where Hid="+temp1);
	if(r1>0) JOptionPane.showMessageDialog(null,"Record Inserted");
	//int r1=stmt.executeUpdate("insert into MMaster values("+temp1+",'"+U2+"',"+temp2+",'"+TX1+"',"+temp3+","+temp4+","+temp5+","+temp6+",'"+TX6+"')");
	//(Mid int, Mname varchar(30),MVid int,MAddr text,MContact char(10), MNShares int ,MSRate int,MFarmArea float,MRMDate date);					
						}
					Combo1.setSelectedIndex(-1);Combo2.setSelectedIndex(-1);Combo3.setSelectedIndex(-1);
						T1.setText("");T2.setText("");T3.setText("");T4.setText("");T5.setText("");
							}
